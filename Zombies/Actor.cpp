...
Actor::Actor(string filename, float x, float y, int width, int height, Game* game) {
	...
	this->width = width;
	this->height = height;
	this->staticActorCollisionFlags = 0;
	this->dynamicActorCollidesWith = COLLISION_ALL;
}
...
float Actor::distanceTo(float x, float y) {
	return Game::distanceBetween(this->x, this->y, x, y);
}

float Actor::distanceTo(Actor* actor) {
	return distanceTo(actor->x, actor->y);
}

...
void GameLayer::init() {
	...
	deadScoreText = new Text("PUNTUACI�N: 0", WIDTH * 0.5f, HEIGHT * 0.4f, game);
...
void GameLayer::processControls() {
	...
	if (dead) {
		...
		return; // no procesar m�s controles de juego si el jugador muri�
	}
	...
	if (controlBuy) {
		...
		if (!doorOpened) {
			// si no abri� ninguna puerta, intentar comprar un arma
...
void GameLayer::update() {
	...
	for (auto window : windows) {
		...
		if (window->barrierHealth < WINDOW_BARRIER_MAX_HEALTH && window->isActorInRebuildBarrierRange(player)) {
			showHelpText("Mant�n [E] para reparar la barrera");
		}
	}
	...
	if (!dead) {
		...
		if (dead) { // el jugador acaba de morir, prepara la pantalla de muerte y actualiza la puntuaci�n record
			if (score > scoreRecord)
				scoreRecord = score;

			deadScoreText->content = "PUNTUACI�N: " + to_string(score);
...
void GameLayer::calculateScroll() {
	...
	if (cameraShakeTime > 1) {
		float shakeAngle = (rand() / (float)RAND_MAX) * 2.0f * M_PI; // un �ngulo aleatorio en radianes
